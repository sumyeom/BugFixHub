...
1. 기획 및 설계
   - API 및 ERD 기획/설계 : 프로젝트 요구사항을 바탕으로 주요 API 및 DB 구조 설계
2. 기능 개발 및 검증
   - Friend 기능 개발: 친구 추가, 추가 된 친구 요청 전환, 친구 전체 조회, 친구 삭제 로직 구현
   - CommentLike 기능 개발: 댓글 좋아요/취소 및 Post/Comment 관계 설정 로직 구현
   - 로직 검증: Postman 및 IntelliJ DB를 통해 구현된 기능 cross 검증
3. 디버깅 및 최적화
   - 버스 수정 및 디버깅: 기존 기능에서 발견된 오류를 수정하고 동작 확인
4. 협업 및 커뮤니케이션
   - 팀원 협의 및 조율: Code Convention 및 Github Rules 협의
5. 소스 관리
   - 개인 Branch를 생성하고, Pull Request -> Merge하여 팀원들과의 소스 관리 작업 수행
6. 문서화 및 유지보수성 강화
